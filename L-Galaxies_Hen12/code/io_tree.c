...
#ifdef LOADIDS
#ifdef MCMC
  sprintf(buf, "%s/MergerTrees_%d/treedata/tree_dbids_%03d.%d", SimulationDir, ThisTask, LastDarkMatterSnapShot, filenr);
#else
#ifndef MRII
  sprintf(buf, "%s/treedata/tree_dbids_%03d.%d", SimulationDir, LastDarkMatterSnapShot, filenr);
#else
  sprintf(buf, "%s/treedata/tree_sf1_dbids_%03d.%d", SimulationDir, LastDarkMatterSnapShot, filenr);
#endif
#endif
...
#ifdef MCMC
  sprintf(buf, "%s/MergerTrees_%d/treedata/trees_%03d.%d", SimulationDir, ThisTask, LastDarkMatterSnapShot, filenr);
#else
#ifndef MRII
  sprintf(buf, "%s/treedata/trees_%03d.%d", SimulationDir, LastDarkMatterSnapShot, filenr);
#else
  sprintf(buf, "%s/treedata/trees_sf1_%03d.%d", SimulationDir, LastDarkMatterSnapShot, filenr);
#endif
#endif
...
void load_tree(int nr)
{
  ...
  if(AllocValue_MaxHaloGal == 0)
    AllocValue_MaxHaloGal = 1 + TreeNHalos[nr] / (0.25 * MAXSNAPS);
