  ...
  if(Gal[p].ColdGas > 0.0)
    {
      ...
      /*redshift dependence */ //Removed Feb 2014 due to replaced metals- Scott Clay
...
#ifdef METALS //Edited to use observation metallicities instead - Scott Clay Feb 2014
      //Zg = Gal[p].MetalsColdGas.type2/Gal[p].ColdGas/0.02;
      Zg = metals_total(Gal[p].MetalsColdGas)/Gal[p].ColdGas/0.02;
      //Zg = pow(10,-0.0864*(pow(log10((Gal[p].DiskMass+Gal[p].BulgeMass)*1.0e10)-12.76,2)+8.79-12+3.1)); //modela z>3.5
      //Zg = pow(10,-0.0864*(pow(log10((Gal[p].DiskMass+Gal[p].BulgeMass)*1.0e10)-12.87,2)+8.90-12+3.1));	  //modelb z>3.5
      //Zg = pow(10,-0.0864*(pow(log10((Gal[p].DiskMass+Gal[p].BulgeMass)*1.0e10)-11.18,2)+9.04-12+3.1));     //z=0.07
#else
      //Zg = Gal[p].MetalsColdGas/Gal[p].ColdGas/0.02;
      Zg = metals_total(Gal[p].MetalsColdGas)/Gal[p].ColdGas/0.02;
      //Zg = pow(10,-0.0864*(pow(log10((Gal[p].DiskMass+Gal[p].BulgeMass)*1.0e10)-12.76,2)+8.79-12+3.1)); //modela z>3.5
      //Zg = pow(10,-0.0864*(pow(log10((Gal[p].DiskMass+Gal[p].BulgeMass)*1.0e10)-12.87,2)+8.90-12+3.1));	  //modelb z>3.5
      //Zg = pow(10,-0.0864*(pow(log10((Gal[p].DiskMass+Gal[p].BulgeMass)*1.0e10)-11.18,2)+9.04-12+3.1));     //z=0.07
      ...
      for(k = 0; k < NMAG; k++)
        {
    	  ...
    	  tauvbc = tauv * (1. / mu - 1.);
    	  taubc = tauvbc * pow(FilterLambda[k] / 0.55, -0.7);
      ...
      for(k = 0; k < NMAG; k++)
        {
    	  ...
    	  // now remove light from young stars absorbed by birth clouds
    	  tauvbc = tauv * (1. / mu - 1.);
    	  taubc = tauvbc * pow((FilterLambda[k] * (1. + ZZ[ListOutputSnaps[snap]])) / 0.55, -0.7);
    	  ...
    	  // now remove light from young stars absorbed by birth clouds
    	  if(snap < MAXSNAPS - 1)
    		taubc = tauvbc * pow((FilterLambda[k] * (1. + ZZ[ListOutputSnaps[snap] + 1])) / 0.55, -0.7);
    	  else
    		taubc = tauvbc * pow((FilterLambda[k] * (1. + ZZ[ListOutputSnaps[snap]])) / 0.55, -0.7);
