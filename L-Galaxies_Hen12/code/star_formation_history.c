 ...
 * double SFH_t[MAXSNAPS][STEPS][SFH_NBIN]; //Time to present at the low-z edge of the bin (code units)
...
void sfh_merge(int p, int p1)
{
  ...
  for(i=0;i<=Gal[p].sfh_ibin;i++) {
    ...
    Gal[p].sfh_BulgeMass[i]+=Gal[p1].sfh_BulgeMass[i]; //Lizhi: Corrected to add mass from p1 to p. (25-04-12)
    Gal[p].sfh_ICM[i]+=Gal[p1].sfh_ICM[i]; //Lizhi: Corrected to add mass from p1 to p. (25-04-12)
...
void create_sfh_bins() {
  ...
  for(snap=0;snap<MAXSNAPS-1;snap++) {
