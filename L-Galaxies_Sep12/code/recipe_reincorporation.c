...
void reincorporate_gas(int p, double dt)
{
    ...
    * code Mvir is already in units of 1.e10*/
 	  ...
 	  if(ReIncorporationRecipe == 2)
 	  {
 	  	...
 	  	reincorporated = Gal[p].EjectedMass / reinc_time * dt;
 	 	  ...
 	 	  //reinc_time= ReIncorporationFactor/UnitTime_in_years * pow((1+ZZ[Gal[p].SnapNum]),ReincZpower) * pow(Gal[p].Vvir/220.,-ReincVelocitypower) / (hubble_of_z(Gal[p].HaloNr));
 	  	//reincorporated = Gal[p].EjectedMass / reinc_time  *dt ;
  ...
  if (Gal[p].EjectedMass > 0.) {
    fraction=reincorporated/Gal[p].EjectedMass;
    transfer_gas(p,"Hot",p,"Ejected",fraction);
  }
