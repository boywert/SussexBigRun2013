...
void Senna()
{
  ...
  if (Sample_Cosmological_Parameters==0) initialize_halomodel();
#endif
   ...
   sprintf(buf, "%s/senna_g%c_%d.txt", OutputDir, DirNum, ThisTask);
  ...
  for(ii=1;ii<ChainLength+1;ii++)
	{
	  ...
	  time(&final);
	  printf("Task %d chain %d (%d) took %lds\n", ThisTask, CurrentMCMCStep, GlobalMCMCStep, final - local_initial);
	  ...
	  {
	    sprintf(buf, "%s %s %s", JobSubmitCommand, JobSubmitPipe, JobSubmitFile);
			...
			if(strcmp(MCMC_PAR[i].Type,"Physical")==0)
			{
				if(Sample_Physical_Parameters==1)
				{
					if(Time_Dependant_PhysPar==1)
						for(snap=0;snap<NOUT;snap++)
							fprintf(fmcmc," %0.3g", log10(MCMC_PAR[i].Value[snap]));
					else
						fprintf(fmcmc," %0.3g", log10(MCMC_PAR[i].Value[0]));
				}
			}
			...
			if(MCMC_PAR[i].Sampling_Switch==1)
			{
				if (strcmp(MCMC_PAR[i].Type,"Physical")==0)
				{
					if(Sample_Physical_Parameters==1)
					{
						if(Time_Dependant_PhysPar==1)
							for(snap=0;snap<NOUT;snap++)
								printf("%0.3g ",MCMC_PAR[i].PropValue[snap]);
						else
							printf("%0.3g ",MCMC_PAR[i].PropValue[0]);
					}
				}
				else if (strcmp(MCMC_PAR[i].Type,"Cosmological")==0)
				{
			...
			if(MCMC_PAR[i].Sampling_Switch==1)
			{
				if (strcmp(MCMC_PAR[i].Type,"Physical")==0)
				{
					if(Sample_Physical_Parameters==1)
					{
						if(Time_Dependant_PhysPar==1)
							for(snap=0;snap<NOUT;snap++)
								printf("%0.3g ",log10(MCMC_PAR[i].PropValue[snap]));
						else
							printf("%0.3g ",log10(MCMC_PAR[i].PropValue[0]));
					}
				}
				else if (strcmp(MCMC_PAR[i].Type,"Cosmological")==0)
				{
...
void initialize_mcmc_par_and_lhood (FILE *fmcmc)
{
  ...
  if(jj==0)
  {
  	sprintf(buf, "%s/senna_00.txt", OutputDir);
...
void open_files_with_comparison_to_observations()
{
	...
	sprintf(buf, "%sMCMC_LIKELIHOOD_%d.txt", OutputDir, ThisTask);
			...
			for(snap=0;snap<NOUT;snap++)
			{
				if(MCMC_Obs[constraint].ObsTest_Switch_z[snap]==1)
				{
					sprintf(buf, "%sPredictionsPerStep_%s_z%1.2f_%d.txt", OutputDir, MCMC_Obs[constraint].Name,
							(double)((int)((MCMCConstraintsZZ[snap]*10)+0.5)/10.), ThisTask);
