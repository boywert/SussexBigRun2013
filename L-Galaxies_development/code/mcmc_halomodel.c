...
void init_numgal(float masslimit_low, float masslimit_high, int snap) {
  ...
  for (mbin=0; mbin<massbins; ++mbin) {
    for (j=0; j<UsedFofsInSample[snap]; ++j) {
      ...
      if (MCMC_FOF2[j].M_Mean200[snap]>minfofmass+mbin*(maxfofmass-minfofmass)/(double)massbins && MCMC_FOF2[j].M_Mean200[snap]<minfofmass+(mbin+1)*(maxfofmass-minfofmass)/(double)massbins) {
