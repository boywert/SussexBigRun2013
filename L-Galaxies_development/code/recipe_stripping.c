...
void deal_with_satellites(int centralgal, int ngal)
{
  ...
  for(i = 0; i < ngal; i++)    /* Loop over all galaxies in the FoF-halo */
  {
...
#else
			  transfer_hot_gas(centralgal,"Hot",i,"Hot",1.);
			  transfer_hot_gas(centralgal,"Ejected",i,"Ejected",1.);
#endif
...
#else
				transfer_hot_gas(Gal[i].CentralGal,"Hot",i,"Hot",1.);
				transfer_hot_gas(Gal[i].CentralGal,"Ejected",i,"Ejected",1.);
#endif
#ifdef TRACK_BURST
				/* Transfer burst component first */
	  ...
	  else if (HotGasStripingModel == 1)
	  {
	  	...
	  	if (Gal[i].Type ==2 && Gal[i].HotGas > 0.0 && HotGasOnType2Galaxies==0)
	  	{
...
#else
			  transfer_hot_gas(Gal[i].CentralGal,"Hot",i,"Hot",gasfraction_intotype1);
			  transfer_hot_gas(Gal[i].CentralGal,"Ejected",i,"Ejected",gasfraction_intotype1);
#endif
...
#else
		  		transfer_hot_gas(merger_centre,"Hot",i,"Hot",stripped_fraction);
		  		transfer_hot_gas(merger_centre,"Ejected",i,"Ejected",stripped_fraction);
#endif
