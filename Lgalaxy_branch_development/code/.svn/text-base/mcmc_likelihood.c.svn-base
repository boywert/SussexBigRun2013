 ...
 ******************************/
				if(strcmp(MCMC_Obs[i].TestType,"chi_sq")==0)
				{
					...
					for(j = 0; j < TotMCMCGals[snap]; j++)
					{
						...
						{
							if((double)((int)((MCMCConstraintsZZ[snap]*10)+0.5)/10.)==0.) //Bj band at z=0
						...
						{
							if((double)((int)((MCMCConstraintsZZ[snap]*10)+0.5)/10.)==0.) //z=0
							{
								if( (MCMC_GAL[j].Magu[snap]-MCMC_GAL[j].Magr[snap]) > (2.06-0.244*tanh((MCMC_GAL[j].Magr[snap]+20.07)/1.09)))
									samdata[j] = MCMC_GAL[j].StellarMass[snap];
							}
							else //z>0
							{
...
//	printf("Nfofs=%d\n",NFofsInSample[snap]);
		...
		if(MCMC_FOF[fof].NGalsInFoF[snap]>0)
		{
			...
			for(jj=0;jj<TotMCMCGals[snap];jj++)
				if(MCMC_GAL[jj].fofid[snap] == fof)
				{
					...
					MCMC_GAL[jj].fofmass[snap] = MCMC_FOF[fof].M_Crit200[snap];
				}
			currentNfof++;
		}

	NFofsInSample[snap]=TotalNFof;
	...
	//	for(fof=0;fof<1; fof++)
	{
		...
		for(jj=0;jj<TotMCMCGals[snap];jj++)
		{
			if(MCMC_GAL[jj].fofid[snap]==fof)
			{
			  //if type 0 get the first place in hashtable for this group (CumulativeNgals[fof])
...
void compute_correlation_func(int ObsNr, double *binsamdata, int snap, float mingalmass, float maxgalmass)
{
	...
	gsl_spline_init(Proj_Spline,r,proj,NR);
	 ...
	 for(ii=0;ii<Nbins[snap][ObsNr]-1;ii++)
		 binsamdata[ii]=gsl_spline_eval(Proj_Spline,MCMC_Obs[ObsNr].Bin[snap][ii],Proj_SplineAcc);

#ifndef  PARALLEL
	sprintf(buf, "/galformod/scratch/bmh20/plots/data/correlation_full_aux_100_%0.2f_%0.2f.txt",mingalmass,maxgalmass);
	if(!(fa = fopen(buf, "w")))
	{
		char sbuf[1000];
	...
	for(ii=0;ii<Nbins[snap][ObsNr]-1;ii++)
		fprintf(fa, "%g %g %g\n", MCMC_Obs[ObsNr].Bin[snap][ii],binsamdata[ii],binsamdata[ii]*0.1);
	fclose(fa);
#endif
...
void bin_function(int ObsNr, double *binsamdata, double *samdata, int snap)
{
...
#ifndef PARALLEL
  sprintf(buf, "/galformod/scratch/bmh20/plots/data/mcmc_plus_obs%d_z%1.2f.txt",ObsNr,(double)((int)((MCMCConstraintsZZ[snap]*10)+0.5)/10.));
 	  if(!(fa = fopen(buf, "w")))
 	    {
 		  char sbuf[1000];
 		  sprintf(sbuf, "can't open file `%s'\n", buf);
  ...
  if((MCMC_Obs[ObsNr].Bin[snap][1] - MCMC_Obs[ObsNr].Bin[snap][0]) > 0.)
    Bin = MCMC_Obs[ObsNr].Bin[snap][1] - MCMC_Obs[ObsNr].Bin[snap][0];
  else
    Bin = MCMC_Obs[ObsNr].Bin[snap][0] - MCMC_Obs[ObsNr].Bin[snap][1];
  ...
  for(i = 0; i < Nbins[snap][ObsNr]; i++)
    {
      ...
      for(k=0;k<TotMCMCGals[snap];k++)
    	  	if(samdata[k]>=(MCMC_Obs[ObsNr].Bin[snap][i]-Bin/2.)
    	  			&& samdata[k] <= (MCMC_Obs[ObsNr].Bin[snap][i]+Bin/2.)) binsamdata[i]+=MCMC_GAL[k].Weight[snap];
...
#ifndef PARALLEL
      fprintf(fa, "%g %g %g %g\n", MCMC_Obs[ObsNr].Bin[snap][i], MCMC_Obs[ObsNr].Obs[snap][i],
  ...
  for(i = 0; i < Nbins[snap][ObsNr]; i++)
    {
      ...
      for(k=0;k<TotMCMCGals[snap];k++)
      {
      	...
      	if((double)((int)((MCMCConstraintsZZ[snap]*10)+0.5)/10.)==0.0)
      	{
      	...
      	if(IsRedGalaxy == 1)
      	{
      		if(MCMC_GAL[k].StellarMass[snap]>=(MCMC_Obs[ObsNr].Bin[snap][i]-Bin/2.0)
      				&& MCMC_GAL[k].StellarMass[snap] <= (MCMC_Obs[ObsNr].Bin[snap][i]+Bin/2.0)) red+=MCMC_GAL[k].Weight[snap];
      	}
      	else
      	{
      		if(MCMC_GAL[k].StellarMass[snap]>=(MCMC_Obs[ObsNr].Bin[snap][i]-Bin/2.0)
      				&& MCMC_GAL[k].StellarMass[snap] <= (MCMC_Obs[ObsNr].Bin[snap][i]+Bin/2.0)) blue+=MCMC_GAL[k].Weight[snap];
      	}
      }
      ...
      if((blue + red) > 0)
      	...
      	binredfraction[i] = 0.;
      //printf("mass=%f obs =%f samcolors=%f\n",MCMC_Obs[ObsNr].Bin[snap][i], MCMC_Obs[ObsNr].Obs[snap][i], binredfraction[i]);
    }
}
  ...
  for(i = 0; i < Nbins[snap][ObsNr]; i++)
    {
      ...
      for(k=0;k<TotMCMCGals[snap];k++)
      {
      	...
      	if(IsPassiveGalaxy == 1)
    	  		{
    	    		if(MCMC_GAL[k].StellarMass[snap]>=(MCMC_Obs[ObsNr].Bin[snap][i]-Bin/2.0)
    	    			&& MCMC_GAL[k].StellarMass[snap] <= (MCMC_Obs[ObsNr].Bin[snap][i]+Bin/2.0)) passive += MCMC_GAL[k].Weight[snap];
    	  		}
    	    else
    	    	{
    	    		if(MCMC_GAL[k].StellarMass[snap]>=(MCMC_Obs[ObsNr].Bin[snap][i]-Bin/2.0)
    	    			&& MCMC_GAL[k].StellarMass[snap] <= (MCMC_Obs[ObsNr].Bin[snap][i]+Bin/2.0)) active += MCMC_GAL[k].Weight[snap];
    	    	}
      }
      ...
      if((passive + active) > 0)
      	...
      	binpassivefraction[i] = 0.;
      //printf("mass=%f obs =%f samcolors=%f\n",MCMC_Obs[ObsNr].Bin[snap][i], MCMC_Obs[ObsNr].Obs[snap][i], binpassivefraction[i]);
    }
}
  ...
  for(i = 0; i < Nbins[snap][ObsNr]; i++)
    {
      ...
      for(k=0;k<TotMCMCGals[snap];k++)
      {
    	  ...
    	  if(color>=(MCMC_Obs[ObsNr].Bin[snap][i]-Bin/2.)
    			  && color <= (MCMC_Obs[ObsNr].Bin[snap][i]+Bin/2.)
    			  && MCMC_GAL[k].StellarMass[snap] > 9.0 && MCMC_GAL[k].StellarMass[snap] < 9.5)
    		  bincolorhist[i]=bincolorhist[i]+MCMC_GAL[k].Weight[snap];
      }
      //printf("%f %f %f\n",MCMC_Obs[ObsNr].Bin[snap][i], MCMC_Obs[ObsNr].Obs[snap][i],bincolorhist[i]);
    }
	...
	--df;
      else
        {
         ...
         // printf("OBS[%d] snap=%d bin=%f sam=%f obs=%f error=%f chsq=%f\n",
          	//	ObsNr, MCMC_Obs[ObsNr].Bin[snap][i], snap,samdata[i],obs, obserror,(temp*temp)/(obserror*obserror));
        }
    }
...
{
  double fracterr = 0.05, probaux = 99.0, prob = 99.0;
  int i;
  double obs, obserror;
