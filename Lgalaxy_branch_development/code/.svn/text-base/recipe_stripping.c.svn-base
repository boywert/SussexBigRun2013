...
{
  int i;
  double dis, fraction, fraction_type1, fraction_type0;
  ...
  for(i = 0; i < ngal; i++)    /* Loop over all galaxies in the FoF-halo */
    {
	  ...
	  else if (SatelliteRecipe == 1)
	  {
	  	...
	  	if (Gal[i].Type ==2 && Gal[i].HotGas > 0.0)
	  	{
			  ...
			  //fraction_type1=fraction*Gal[Gal[i].CentralGal].HotRadius / Gal[Gal[i].CentralGal].Rvir;
			  else
    			fraction=1.;
			  	//fraction_type1=fraction;
			  ...
			  transfer_gas(Gal[i].CentralGal,"Hot",i,"Hot",fraction_type1);
			  transfer_gas(Gal[i].CentralGal,"Ejected",i,"Ejected",fraction_type1);
			  mass_checks("deal_with_satellites i #0",i);
			  mass_checks("deal_with_satellites Gal[i].CentraGal #0",Gal[i].CentralGal);
#ifdef TRACK_BURST
			  /* Transfer burst component first */
			  transfer_stars(Gal[i].CentralGal,"Burst",i,"Burst",
					 fraction*Gal[i].ICM/(Gal[i].DiskMass+Gal[i].BulgeMass+Gal[i].ICM));
#endif
			  ...
			  if (fraction < 1.)
			  	//if (fraction_type1<fraction)
    		    {
			  		//fraction_type0=fraction-fraction_type1;
			  		fraction_type0=1.;
				  transfer_gas(centralgal,"Hot",i,"Hot",fraction_type0);
				  transfer_gas(centralgal,"Ejected",i,"Ejected",fraction_type0);
#ifdef TRACK_BURST
				  /* Transfer burst component first */
				  transfer_stars(centralgal,"Burst",i,"Burst",
					 Gal[i].ICM/(Gal[i].DiskMass+Gal[i].BulgeMass+Gal[i].ICM));
#endif
				  transfer_stars(centralgal,"ICM",i,"ICM",fraction_type0);
...
{
  double hotremain;
  double R_Stripping, R_Tidal, R_RamPressure, R_Orbit;
  double RetainFrac;
