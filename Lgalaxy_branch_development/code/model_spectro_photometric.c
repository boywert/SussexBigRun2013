...
void setup_LumTables_precomputed(void)
{
  FILE *fa, *fb;
  int i, MetalLoop, AgeLoop, band, snap;
  ...
  for(MetalLoop = 0; MetalLoop < SSP_NMETALLICITES; MetalLoop++)
  {
  	...
  	for(band = 0; band < NMAG; band++)
  	{
  		...
  		//READ TABLES
  		sprintf(buf, "%s/PhotTables/%s_%s_Phot_Table_%s_Mag%s_m%0.4f.dat", SpecPhotDir, PhotPrefix, SpecPhotIMF, Simulation, FilterName,
  				pow(10,SSP_logMetalTab[MetalLoop]));
  		//sprintf(buf, "/galformod/scratch/bmh20/Workspace/test_phot_tables/%s_%s_Phot_Table_%s_Mag%s_m%0.4f.dat", PhotPrefix, SpecPhotIMF, Simulation, FilterName,
  		 // 				pow(10,SSP_logMetalTab[MetalLoop]));
  		if(!(fb = fopen(buf, "r")))
  		{
  			char sbuf[1000];
  		...
  		/* check that the numbers on top of the file correspond to MAXSNAPS and SSP_NAGES */
  		if(FilterLambda[band] != dumb_filterlambda)
  		{
  			terminate("filterlambdas dont match");
  		}
  		if(dumb_ssp_nsnaps != MAXSNAPS)
  		{
...
void setup_Spec_LumTables_onthefly(void)
{
	...
	//2nd Loop on redshift
	      for(snap=0;snap<MAXSNAPS;snap++)
	      {
	      	redshift=RedshiftTab[MAXSNAPS-snap-1];
