...
{
  int i, ngroups, nids, groupcount, nFiles, nsubhalos, subcount;
  MyIDType idcount;
  char buf[1000];
  unsigned int j;
  int* GroupLen;
  ...
  for(i = 0; i < nFiles; i++) {
    ...
    hid_t   fd, hd, attr, sr, id, dset,gr;
    herr_t  ret; 
    ...
    if(i == 0)
      {
	GroupOffset = mymalloc(sizeof(int) * cat->TotNgroups);
	GroupOffset[0] = 0;
    ...
    if(ngroups > 0) {
      ...
      ret = H5Dread (dset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &GroupLen[groupcount]);
      ret = H5Dclose(dset);
      dset = H5Dopen (gr, "GroupFirstSub");
      ret = H5Dread (dset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &GroupFirstSub[groupcount]);
      ret = H5Dclose(dset);
      dset = H5Dopen (gr, "GroupNsubs");
      ret = H5Dread (dset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &GroupNsubs[groupcount]);
      ret = H5Dclose(dset);
      ret = H5Gclose(gr);
      for(j = 0; j < ngroups; j++)
	if ((groupcount + j) > 0)
