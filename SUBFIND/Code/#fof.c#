...
#ifdef KD_SEED_STAR_MASS_FRACTION
      if(num == -2) printf("\n      linking on star particles for BH seeding (prim=%d, sec=%d, Nmin=%d) ...",MyFOF_PRIMARY_LINK_TYPES,MyFOF_SECONDARY_LINK_TYPES,MyFOF_GROUP_MIN_LEN);
#endif
...
#ifdef OMP_SORT
  omp_qsort(FOF_GList, NgroupsExt, sizeof(struct fof_group_list), fof_compare_FOF_GList_MinIDTask);
#else
...
#ifdef OMP_SORT
  omp_qsort(FOF_GList, NgroupsExt, sizeof(struct fof_group_list), fof_compare_FOF_GList_MinID);
  omp_qsort(get_FOF_GList, nimport, sizeof(struct fof_group_list), fof_compare_FOF_GList_MinID);
#else
...
#ifdef OMP_SORT
  omp_qsort(get_FOF_GList, nimport, sizeof(struct fof_group_list), fof_compare_FOF_GList_MinIDTask);
  omp_qsort(FOF_GList, NgroupsExt, sizeof(struct fof_group_list), fof_compare_FOF_GList_MinIDTask);
#else
...
#ifdef OMP_SORT
  omp_qsort(Group, NgroupsExt, sizeof(struct group_properties), fof_compare_Group_MinIDTask);
#else
...
#ifdef OMP_SORT
  omp_qsort(Group, NgroupsExt, sizeof(struct group_properties), fof_compare_Group_MinID);
  omp_qsort(get_Group, nimport, sizeof(struct group_properties), fof_compare_Group_MinID);
#else
...
#ifdef OMP_SORT
  omp_qsort(Group, Ngroups, sizeof(struct group_properties), fof_compare_Group_MinID);
#else
  ...
  while(startnode >= 0)
    {
      while(startnode >= 0)
	{
	  numngb_inbox = ngb_treefind_fof_nearest(pos, h, target, &startnode, mode, nexport, nsend_local, MyFOF_PRIMARY_LINK_TYPES);
...
#else
      if(Group[i].MassType[4] > KD_SEED_STAR_MASS_FRACTION * All.MinFoFMassForNewSeed && Group[i].LenType[2] == 0)
#endif
...
#endif
#ifdef KD_SEED_STAR_MASS_FRACTION
      BPP(import_indices[n]).BH_Mass = All.SeedBlackHoleMass * import_fofmass[n] / (KD_SEED_STAR_MASS_FRACTION * All.MinFoFMassForNewSeed);
#else
...
void fof_assign_HostHaloMass(void) /* assigns mass of host FoF group to SphP[].HostHaloMass for SPH particles */ 
{
...
#ifdef OMP_SORT
  omp_qsort(FOF_GList, NgroupsExt, sizeof(struct fof_group_list), fof_compare_FOF_GList_MinIDTask_MinID);
#else
  ...
  for(i = 0; i < N_gas; i++) SphP[i].HostHaloMass = 0;
...
#ifdef OMP_SORT
  omp_qsort(FOF_GList, NgroupsExt, sizeof(struct fof_group_list), fof_compare_FOF_GList_MinID); /* restore original order */
#else
...
void read_fof(int num)
{
  ...
  omp_qsort(ID_list, Nids, sizeof(struct id_list), fof_compare_ID_list_ID);
#else
