...
time_hlp = fltarr(LINES-1)
time_hlp = time[1:Lines-1]
time_hlp -= time[0:Lines-2]

ind = where(time_hlp lt 0.0)
if size(ind, /dimensions) gt 0 then begin
    for i = 0, n_elements(ind)-1 do begin
        tot[ind[i]+1:LINES-1] += tot[ind[i]] - tot[ind[i]+1]
        treegrav[ind[i]+1:LINES-1] += treegrav[ind[i]] - treegrav[ind[i]+1]
        treebuild[ind[i]+1:LINES-1] += treebuild[ind[i]] - treebuild[ind[i]+1]
        treeupdate[ind[i]+1:LINES-1] += treeupdate[ind[i]] - treeupdate[ind[i]+1]
        treewalk[ind[i]+1:LINES-1] += treewalk[ind[i]] - treewalk[ind[i]+1]
        treecomm[ind[i]+1:LINES-1] += treecomm[ind[i]] - treecomm[ind[i]+1]
        treeimbal[ind[i]+1:LINES-1] += treeimbal[ind[i]] - treeimbal[ind[i]+1]
        pmgrav[ind[i]+1:LINES-1] += pmgrav[ind[i]] - pmgrav[ind[i]+1]
        sph[ind[i]+1:LINES-1] += sph[ind[i]] - sph[ind[i]+1]
        density[ind[i]+1:LINES-1] += density[ind[i]] - density[ind[i]+1]
        denscomm[ind[i]+1:LINES-1] += denscomm[ind[i]] - denscomm[ind[i]+1]
        densimbal[ind[i]+1:LINES-1] += densimbal[ind[i]] - densimbal[ind[i]+1]
        hydrofrc[ind[i]+1:LINES-1] += hydrofrc[ind[i]] - hydrofrc[ind[i]+1]
        hydcom[ind[i]+1:LINES-1] += hydcom[ind[i]] - hydcom[ind[i]+1]
        hydimbal[ind[i]+1:LINES-1] += hydimbal[ind[i]] - hydimbal[ind[i]+1]
        hmaxupdate[ind[i]+1:LINES-1] += hmaxupdate[ind[i]] - hmaxupdate[ind[i]+1]
        domain[ind[i]+1:LINES-1] += domain[ind[i]] - domain[ind[i]+1]
        potential[ind[i]+1:LINES-1] += potential[ind[i]] - potential[ind[i]+1]
        predict[ind[i]+1:LINES-1] += predict[ind[i]] - predict[ind[i]+1]
        kicks[ind[i]+1:LINES-1] += kicks[ind[i]] - kicks[ind[i]+1]
        io[ind[i]+1:LINES-1] += io[ind[i]] - io[ind[i]+1]
        peano[ind[i]+1:LINES-1] += peano[ind[i]] - peano[ind[i]+1]
        sfrcool[ind[i]+1:LINES-1] += sfrcool[ind[i]] - sfrcool[ind[i]+1]
        blackholes[ind[i]+1:LINES-1] += blackholes[ind[i]] - blackholes[ind[i]+1]
        fofsubfind[ind[i]+1:LINES-1] += fofsubfind[ind[i]] - fofsubfind[ind[i]+1]
        smoothing[ind[i]+1:LINES-1] += smoothing[ind[i]] - smoothing[ind[i]+1]
        hotngbs[ind[i]+1:LINES-1] += hotngbs[ind[i]] - hotngbs[ind[i]+1]
        weights_hot[ind[i]+1:LINES-1] += weights_hot[ind[i]] - weights_hot[ind[i]+1]
        enrich_hot[ind[i]+1:LINES-1] += enrich_hot[ind[i]] - enrich_hot[ind[i]+1]
        weights_cold[ind[i]+1:LINES-1] += weights_cold[ind[i]] - weights_cold[ind[i]+1]
        enrich_cold[ind[i]+1:LINES-1] += enrich_cold[ind[i]] - enrich_cold[ind[i]+1]
        cs_misc[ind[i]+1:LINES-1] += cs_misc[ind[i]] - cs_misc[ind[i]+1]
        misc[ind[i]+1:LINES-1] += misc[ind[i]] - misc[ind[i]+1]
    endfor
endif
...
tot_hlp = fltarr(LINES-1)
tot_hlp = tot[1:Lines-1]
tot_hlp -= tot[0:Lines-2]

ind = where(tot_hlp lt 0.0)
if size(ind, /dimensions) gt 0 then begin
    for i = 0, n_elements(ind)-1 do begin
        tot[ind[i]+1:LINES-1] += tot[ind[i]] - tot[ind[i]+1]
        treegrav[ind[i]+1:LINES-1] += treegrav[ind[i]] - treegrav[ind[i]+1]
...
plot, time_axis, speed, pos=[0.11,0.7,0.95,0.93], xrange=[0,max(tot)/3600.0], /xstyle, /noerase,XTickformat='(A1)', yrange=[-0.0005,0.02], /ystyle, ytitle='a/h'
...
;polyfill, [tot(0)/3600.0,tot(0)/3600.0,tot(LINES-1)/3600.0,tot(LINES-1)/3600.0],[0,100.0,100.0,0], color=FSC_Color('Lime Green')
polyfill, [0.0,0.0,max(time_axis),max(time_axis)],[0,100.0,100.0,0], color=FSC_Color('Lime Green')
...
;polyfill, [time(0), time(0), time(LINES-1),time(LINES-1)],[0,100.0,100.0,0],color=FSC_Color('Lime Green')
polyfill, [time(0), time(0), max(a_axis), max(a_axis)],[0,100.0,100.0,0],color=FSC_Color('Lime Green')
