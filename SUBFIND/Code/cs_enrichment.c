	  ...
	  if(temp < All.Tcrit_Phase && SphP[i].d.Density * a3inv > All.PhysDensThresh * All.DensFrac_Phase)
	    {
	      ...
	      hot_egy =  SphP[i].ea.EntropyAvg / GAMMA_MINUS1 * pow(SphP[i].da.DensityAvg * a3inv, GAMMA_MINUS1);
 		  ...
 		  if (SphP[i].PromotionAttempts > 100) 
		    {
		      printf("NEVER promoted %d %g %g %d %g %g %g %d\n", P[i].ID, P[i].EnergySN, promotion_energy,  
 			     SphP[i].HotNgbNum, SphP[i].ea.EntropyAvg, critical_entropy, entropynew, SphP[i].PromotionAttempts); 
...
#ifdef CS_ATTEMPTS
		  if(!
		     (P[i].EnergySN > promotion_energy && SphP[i].HotNgbNum > (All.DesNumNgb - All.MaxNumHotNgbDeviation)
