...
void domain_Decomposition(int UseAllTimeBins, int SaveKeys)
{
  ...
  if (UseAllParticles) highest_bin_to_include = All.HighestOccupiedTimeBin;
  else highest_bin_to_include = All.HighestActiveTimeBin;
      ...
      if(GrNr < 0)
	{
	  ...
	  if(SaveKeys != 0)
	      ...
	      if(N_gas > 0)             /* First the Gas particles */
		{
	      ...
	      printf("%d %d %d %d\n",NPartPerKey[0],NPartPerKey[1],NPartPerKey[2],NPartPerKey[3]);
	      ...
	      if(NumPart > N_gas)       /* Now the rest */
		{
		  for(k = 1; k < 6; k++)
		      ...
		      else
			{
			  ...
			  printf("k=%d, key_count=%d, previous_keys=%d, offset=%d\n",k,key_count,previous_keys,offset);
...
#endif
{
  ...
  if(domain_determineTopTree(SaveKeys))
    return 1;
#else
  domain_findExtent();
...
void domain_exchange(void)
{
  ...
  if(count_totget)
    {
      memmove(P + N_gas + count_totget, P + N_gas,
	      (NumPart - N_gas) * sizeof(struct particle_data));
      memmove(Key + N_gas + count_totget, Key + N_gas,
...
void domain_assign_load_or_work_balanced(int mode)
{
  ...
  for(q = 0; q < 3; q++)
    {
      queues[q].next = mymalloc("queues[q].next", NTask * sizeof(int));
      queues[q].previous = mymalloc("queues[q].previous", NTask * sizeof(int));
      queues[q].value = mymalloc("queues[q].value", NTask * sizeof(double));
	  ...
	  do
	      ...
	      for(ta = 0; ta < NTask; ta++)
		{
		  ...
		  MPI_Bcast(&count_toget_BHs, 1, MPI_INT, ta, MPI_COMM_WORLD);
                  ...
                  ntoomany = (list_N_gas[ta] + count_toget_sph - count_togo_sph - All.MaxPartSph) > 0;
                  ifntoomany = ntoomany;
                  
#ifdef LT_STELLAREVOLUTION
                  ntoomanystars = (list_N_stars[ta] + count_toget_stars - count_togo_stars - All.MaxPartMet) > 0;
                  ifntoomany = ntoomany || (ntoomanystars > 0);
#endif
#if defined(BLACK_HOLES) && defined(DETACH_BLACK_HOLES)
                  ntoomanyBHs = (list_N_BHs[ta] + count_toget_BHs - count_togo_BHs - All.MaxPartBH) > 0;
                  ifntoomany = ntoomany || (ntoomanyBHs > 0);
#endif
		      ...
		      if(ThisTask == 0)
			{
			  ...
			  if(ntoomanyBHs)
				...
				("exchange needs to be modified because I can't receive %d BH-particles on task=%d\n",
				 ntoomanyBHs, ta);
			      if(flagsum > 25)
				printf("list_N_BHs[ta=%d]=%d  count_toget_BHs=%d count_togo_BHs=%d\n",
				       ta, list_N_BHs[ta], count_toget_BHs, count_togo_BHs);
			      fflush(stdout);
			    }
#endif
			}
 ...
 */
#ifdef WRITE_KEY_FILES
int domain_determineTopTree(int SaveKeys)
#else
