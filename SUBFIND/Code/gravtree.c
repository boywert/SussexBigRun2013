...
#endif
#ifdef MODGRAV
	      GravDataIn[j].beta = P[place].mg_beta;
#endif
...
#ifdef MODGRAV
	      for(k = 0; k < 3; k++)
		P[place].ModGravAccel[k] += 2.0*P[place].mg_beta * GravDataOut[j].mg_grad_phi_tree[k];
#endif
...
#ifdef MODGRAV
	  for(j = 0; j < 3; j++)
	    P[i].ModGravAccel[j] += fac * P[i].Pos[j];
#endif
	}
    }
#endif
...
/* note there is no acceleration in z */
  ...
  if(TakeLevel >= 0)
  {
    ...
    if(ThisTask == 0) printf("relative error in the total number of tree-gravity interactions = %g\n",(sum_costtotal-sum_costtotal_new)/sum_costtotal);
...
void sum_top_level_node_costfactors(void)
{
  ...
  double *costlist  = mymalloc("costlist", NTopnodes * sizeof(double));
  double *costlist_all  = mymalloc("costlist_all", NTopnodes * sizeof(double));
...
#ifdef MODGRAV
  All.MinModGravHsml_0 = All.MinModGravHsmlFractional_0 * All.ForceSoftening[0];
  All.MinModGravHsml_1 = All.MinModGravHsmlFractional_1 * All.ForceSoftening[1];
#endif
