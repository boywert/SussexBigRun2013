...
{
  fprintf(stderr," An HDF5 error was detected, we'd better stop.\n");
  H5Eprint (stderr);
  endrun(124);
...
hid_t H5Dcreate_compressed(hid_t handle, char *dataset_name, hid_t hdf5_datatype, hid_t dataspace_id, int rank, hsize_t *dims)
{
  hsize_t chunksize[2];
  int i;
  hid_t property_list, dataset_id;
  
  for(i = 0; i < rank; i++)
    {
      chunksize[i] = dims[i];
      if (chunksize[i] > CHUNKSIZE) chunksize[i] = CHUNKSIZE;
    }

  property_list = H5Pcreate(H5P_DATASET_CREATE);

  /* interlace a block of data by putting most significant bits first to help compression  */
  H5Pset_shuffle(property_list);

  /* set level of gzip compression */
  H5Pset_deflate(property_list, GZIP);

  /* determine sizes of chunks to write */
  H5Pset_chunk(property_list, rank, chunksize);

  dataset_id = H5Dcreate(handle, dataset_name, hdf5_datatype, dataspace_id, property_list);

  H5Pclose(property_list);

  return dataset_id;
}
#endif /* End of HDF5_COMPRESSED_SNAPSHOT conditional */
#endif /* End of HDF5 conditional */
...
void fill_write_buffer(enum iofields blocknr, int *startindex, int pc, int type)
{
  ...
  double *Metallicities, U, DZ, redshift;
#endif
...
#ifdef MAGNETIC
#ifndef MU0_UNITY
      gadget2gauss /= sqrt(All.UnitTime_in_s * All.UnitTime_in_s * All.UnitLength_in_cm / All.UnitMass_in_g / (All.HubbleParam * All.HubbleParam));
#endif
...
/* :: This consider the possibility to include low T metal cooling without using non-eq calculations (only in Luca's cooling):: */
	...
	if(P[pindex].Type == type)
	  {
            double dmax1, dmax2;
            set_metallicities(pindex, Metallicities, a3inv);
            U = DMAX(All.MinEgySpec,
                     (SphP[pindex].Entropy / GAMMA_MINUS1 * pow(SphP[pindex].d.Density * a3inv, GAMMA_MINUS1)));
            U *= All.UnitEnergy_in_cgs / All.UnitMass_in_g;
...
void get_Tab_IO_Label(enum iofields blocknr, char *label)
{
  switch (blocknr)
    ...
    case IO_AMDC:
      ...
      break;
    case IO_ALFA2_DYN:
      strncpy(label, "ADYN", 4); //POSSIBLE CHANGE?
      break;
    case IO_PHI:
      strncpy(label, "PHI ", 4);
      break;
    ...
    case IO_RADGAMMA:
      ...
      break;
    case IO_nHeII:
      strncpy(label, "nHe2", 4);
      break;
    case IO_nHeIII:
      strncpy(label, "nHe3", 4);
      break;
    case IO_EDDINGTON_TENSOR:
      strncpy(label, "ET", 4);
      break;
...
#define SKIP_POT  {my_fwrite(&blksize,sizeof(int),1,fd_pot);}
  ...
  printf("Total Keys per file = %u\n",TotNumKeysFile);
  printf("Task %d: NKeys=%d,%d,%d,%d,%d,%d\n",ThisTask,NKeys[0],NKeys[1],NKeys[2],NKeys[3],NKeys[4],NKeys[5]);
  printf("Task %d: FKeys=%d,%d,%d,%d,%d,%d\n",ThisTask,NKeys_file[0],NKeys_file[1],NKeys_file[2],NKeys_file[3],NKeys_file[4],NKeys_file[5]);
  printf("Task %d: offs =%d,%d,%d,%d,%d,%d\n",ThisTask,write_offset[0],write_offset[1],write_offset[2],write_offset[3],write_offset[4],write_offset[5]);
  ...
  if(npart > 0)
	...
	if(ThisTask == 0)
	  printf(" ... %d bytes = %u x %d (%g MB) ...\n",blksize,TotNumKeysFile,size_of_element,blksize/1024./1024.);
