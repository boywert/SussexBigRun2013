...
double *Metallicities, DZ, Redshift;
#endif
/*
...
/* cooling routine when star formation is enabled */
{
  ...
  Metallicities = (double*)mymalloc("Metallicities", sizeof(double) * get_cool_n_el());
  ...
  for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
      ...
      if(P[i].Type == 0)
	{
	  ...
	  char buffer[1000];
          ...
          set_metallicities(i, Metallicities, a3inv); /* note: sets the physical density too! */
...
/*         um_FillEl_mu = SphP[i].FillEl_mu; */
	  ...
	  Temperature = convert_u_to_temp(uold * All.UnitPressure_in_cgs / All.UnitDensity_in_cgs,
	  ...
	  if(trackwinds_flag != '\0')
	    {
	      ...
	      ignore_failure_in_convert_u = 1;
#if !defined(LT_METAL_COOLING_WAL)              
	      tw_temp = convert_u_to_temp(uold * All.UnitPressure_in_cgs / All.UnitDensity_in_cgs,
					  SphP[i].d.Density * All.UnitDensity_in_cgs * All.HubbleParam *
					  All.HubbleParam * a3inv, &ne_guess);
#else
              tw_temp = convert_u_to_temp(uold * All.UnitPressure_in_cgs / All.UnitDensity_in_cgs,
...
/* 	    printf("\t cooling: flag is %d : %d ** %d\n", flag, ThisTask, All.NumCurrentTiStep); fflush(stdout); */
	  ...
	  if(flag == 1)		/* normal implicit isochoric cooling */
	      ...
	      if(P[i].TimeBin)	/* upon start-up, we need to protect against dt==0 */
		{
	      ...
	      WalCool_set_PID(P[i].ID);
              unew = DoCooling(uold, SphP[i].d.Density * a3inv, Metallicities, Redshift, DZ, dtime);
#else              
#ifdef LT_METAL_COOLING
#ifdef UM_CHEMISTRY
	      unew = Um_DoCooling(uold, SphP[i].d.Density * a3inv, dtime, &ne, Zcool, i, flag);
#else
	      unew = DoCooling(uold, SphP[i].d.Density * a3inv, dtime, &ne, Zcool);
#endif
#else
	  ...
	  if(flag == 0)		/* active star formation */
...
#endif
#else
              tcool = GetCoolingTime(egyhot, SphP[i].d.Density * a3inv, Redshift, DZ, Metallicities);
#endif
		      ...
		      if(SphP[i].i.Injected_BH_Energy > 0)
			{
			  ...
			  if(egycurrent > egyeff)
				...
				tcool = 1e13;
                              ...
                              tcool = GetCoolingTime(egycurrent, SphP[i].d.Density * a3inv, Redshift, DZ, Metallicities);
#endif
		...
		SphP[i].mstar = mstar;
	      ...
	      number_of_stars_generated = (P[i].ID >> (32 - All.StarBits));
	      ...
	      if(get_random_number(P[i].ID + 1) < prob)	/* ok, make a star */
		{
		  ...
		  if((number_of_stars_generated + GENERATIONS_BUNCH) >= All.Generations)
		      ...
		      if(N_stars + 1 >= All.MaxPartMet)
			{
		      ...
		      P[i].ID += (GENERATIONS_BUNCH << (32 - All.StarBits));
		  ...
		  if(get_random_number(P[i].ID + 2) < prob)	/* ok, make the particle go into the wind */
			...
			windv_max = SFs[SFi].WindEnergyFraction;
		      ...
		      tw_temp = convert_u_to_temp(DMAX(All.MinEgySpec,
						       (SphP[i].Entropy + SphP[i].e.DtEntropy * dt) / GAMMA_MINUS1 *
...
#ifdef LT_METAL_COOLING_WAL
  //printf("Ending cooling %d ** %d\n", ThisTask, All.NumCurrentTiStep); fflush(stdout);
  myfree(Metallicities);
#endif
  ...
  *xclouds = 0;
...
#ifdef LT_METAL_COOLING_WAL
  Metallicities = (double*)mymalloc("Metallicities", sizeof(double) * get_cool_n_el());
  set_metallicities(i, Metallicities, a3inv);
  if(All.ComovingIntegrationOn)
    {
  ...
  if(flag == 1)
	...
	*Temperature = convert_u_to_temp(DMAX(All.MinEgySpec,
					     (SphP[i].Entropy + SphP[i].e.DtEntropy * dt) / GAMMA_MINUS1 * pow(SphP[i].d.Density * a3inv,
										  GAMMA_MINUS1)) *
					All.UnitEnergy_in_cgs / All.UnitMass_in_g,
					SphP[i].d.Density * All.UnitDensity_in_cgs * All.HubbleParam *
...
double get_starformation_rate_subfind(int i, float *Temperature, float *xclouds)
{
  ...
  *xclouds = 0;
  ...
  mySFi = get_SF_index_subfind(i, &mySFi, &IMFi);
  IMFp = (IMF_Type *) & IMFs[IMFi];
  SFp = (SF_Type *) & SFs[mySFi];
  myFactorSN = SFs[mySFi].FactorSN;
  myEgySpecSN = SFs[mySFi].EgySpecSN;
...
#ifdef LT_METAL_COOLING_WAL
  Metallicities = (double*)mymalloc("Metallicities", sizeof(double) * get_cool_n_el());
  set_metallicities_subfind(i, Metallicities, a3inv);
  if(All.ComovingIntegrationOn)
    {
      Redshift = 1.0 / All.Time - 1;
      get_cool_redshift(Redshift, &DZ);
...
#else
  if((metalmass = get_metalmass(SphP[P[i].origindex].Metals)) > 0)
    Z = get_metallicity_solarunits(SphP[P[i].origindex].Zsmooth * SphP[P[i].origindex].Metals[Iron] / metalmass);
  else
    Z = NO_METAL;
#endif
#endif
...
#ifndef UM_CHEMISTRY
      ne = SphP[P[i].origindex].Ne;
#else
      ne = SphP[P[i].origindex].elec;
      Um_Compute_MeanMolecularWeight(i);
      u = DMAX(All.MinEgySpec, SphP[P[i].origindex].Entropy / GAMMA_MINUS1 * pow(SphP[P[i].origindex].d.Density * a3inv, GAMMA_MINUS1));
      *Temperature =
	GAMMA_MINUS1 / BOLTZMANN * PROTONMASS * SphP[P[i].origindex].Um_MeanMolecularWeight * u * All.UnitPressure_in_cgs /
	All.UnitDensity_in_cgs;
      if(*Temperature >= T_SUP_INTERPOL_LIMIT)
#endif
...
#ifndef LT_METAL_COOLING_WAL        
	*Temperature = convert_u_to_temp(DMAX(All.MinEgySpec,
					     (SphP[P[i].origindex].Entropy + SphP[P[i].origindex].e.DtEntropy * dt) / GAMMA_MINUS1 * pow(SphP[P[i].origindex].d.Density * a3inv,
										  GAMMA_MINUS1)) *
					All.UnitEnergy_in_cgs / All.UnitMass_in_g,
					SphP[P[i].origindex].d.Density * All.UnitDensity_in_cgs * All.HubbleParam *
					All.HubbleParam * a3inv, &ne);
#else
	*Temperature = convert_u_to_temp(DMAX(All.MinEgySpec,
					     (SphP[P[i].origindex].Entropy + SphP[P[i].origindex].e.DtEntropy * dt) / GAMMA_MINUS1 * pow(SphP[P[i].origindex].d.Density * a3inv,
...
#ifdef LT_METAL_COOLING
#ifndef UM_CHEMISTRY
  if((tcool = GetCoolingTime(egyhot, SphP[P[i].origindex].d.Density * a3inv, &ne, Z)) == 0)
    tcool = 1e13;
#else
...
#endif
#else
  tcool = GetCoolingTime(egyhot, SphP[P[i].origindex].d.Density * a3inv, &ne);
#endif
#else /* LT_METAL_COOLING_WAL */
  tcool = GetCoolingTime(egyhot, SphP[P[i].origindex].d.Density * a3inv, Redshift, DZ, Metallicities);
#endif
#else /* RT_COOLING_PHOTOHEATING */
...
#endif
#else
      if((tcool = GetCoolingTime(egyhot, dens, Redshift, DZ, Metallicities)) == 0)
	tcool = 1e13;
#endif
...
#endif
#else
      tcool = GetCoolingTime(egyhot, dens, Redshift, DZ, Metallicities);
#endif
...
#endif
#else
      tcool = GetCoolingTime(egyhot, rho, Redshift, DZ, Metallicities);
#endif
      y = tsfr / tcool * egyhot / (fSN * egySN - (1 - fSN) * All.EgySpecCold);
      if(y < 1e-4)
	x = y;
...
#endif
#else
              tcool2 = GetCoolingTime(egyhot2, rho2, Redshift, DZ, Metallicities);
#endif
