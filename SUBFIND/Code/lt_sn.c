...
#ifdef OMP_SORT
      omp_qsort(MetalDataIn, nexport, sizeof(struct metaldata_index), metaldata_index_compare);
#else
...
int is_chemically_active(int i)
{
  ...
  if(P[i].Type == 4)
	...
	flag = 1;
      ...
      if(TimeBinActive[MetP[P[i].pt.MetID].ChemTimeBin] && P[i].StellarAge < (MyFloat) All.Time)
