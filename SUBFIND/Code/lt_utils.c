...
#else
/* Star Burst selection */
int INLINE_FUNC get_SF_index_subfind(int i)
{
  switch (All.StarBurstCondition)
    {
    case SB_DENSITY:
      if(SphP[P[i].origindex].a2.Density < All.SB_Density_Thresh)
	*mySFi = 0;
      else
	*mySFi = 1;
      break;
    ...
    case SB_DENTROPY:
      if(SphP[P[i].origindex].e.DtEntropy < All.SB_DEntropy_Thresh)
	*mySFi = 0;
      else
	*mySFi = 1;
      break;
    ...
    case (SB_DENSITY + SB_DENTROPY):
      if(SphP[P[i].origindex].a2.Density >= All.SB_Density_Thresh && SphP[P[i].origindex].e.DtEntropy >= All.SB_DEntropy_Thresh)
	*mySFi = 1;
      else
	*mySFi = 0;
      break;
...
int get_chemstep_bin(double a, double chemstep, int *chem_step, int i)
{
  ...
  if(chemstep<0.0)  // This should happen only when All.Time==1.0, because the look-back time is near 0 */
    {
    if(fabs(All.Time-1.e-6) <1.0) printf("%d says: ! This should occur only at the last time step!\n",ThisTask);
