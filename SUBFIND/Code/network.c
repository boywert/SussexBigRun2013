...
static int network_init_internal( char *speciesfile, char *ratesfile, char *partfile, char *massesfile, char *weakratesfile, struct network_data *nd, int onlyweak ) {
  ...
  while (!feof(fd)) {
    if (fgets( cdummy, 200, fd ) == NULL) break;
  ...
  while (!feof(fd)) {
    if (fgets( cdummy, 200, fd ) == NULL) break; /* skip name of the nucleus */
    if (fgets( cdummy, 200, fd ) == NULL) break;
    sscanf( cdummy, "%d%d%f\n", &nz, &na, &spin );
  ...
  while (!feof(fd)) {
    ...
    if (nucEductId >= 0 && nucProductId >= 0) {
      ...
      for (i=0; i<143; i++) {
        ...
        if (isFirst) {
          if (i < 13) sscanf( &cdummy[0], "%f", &nd->weakTemp[i] );
          if (i % 13 == 0) sscanf( &cdummy[6], "%f", &nd->weakRhoYe[i/13] );
        }
      }
  ...
  while (!feof(fd)) {
    ...
    if (sscanf( cdummy, "%d", &ratetype ) != 1) break;
    safe_fgets( cdummy, 200, fd );
#endif
    for (i=0; i<6; i++) {
    ...
    if (needed) {
      ...
      if (needed) {
        for (i=0; i<nd->rates[rc].ninput; i++) nd->rates[rc].input[i] = nucids[i];
        for (i=0; i<nd->rates[rc].noutput; i++) nd->rates[rc].output[i] = nucids[i+nd->rates[rc].ninput];
      }
    }
    ...
    if (needed) {
	 ...
	 it is a weak rate exactly if the reaction changes the total number of protons */
      zsum = 0;
      for (i=0; i<nd->rates[rc].ninput; i++)  zsum += nd->nucdata[ nd->rates[rc].input[i] ].nz;
      for (i=0; i<nd->rates[rc].noutput; i++) zsum -= nd->nucdata[ nd->rates[rc].output[i] ].nz;
...
int network_getrhs(double rho, double temp, const double y[], int compute_derivs, const struct network_data *nd, struct network_workspace *nw, double *rhs, network_var *deriv) {
  ...
  for (nucdata = nd->nucdata, rhsiter = deriv, pratelist = nw->prates, pweakratelist = nw->pweakrates;
    ...
    for (w=nucdata->w, prate=*pratelist;
         w!=wend;
         w++, prate++) {
    ...
    for (w=nucdata->wweak, prate=*pweakratelist;
         w!=wend;
         w++, prate++) {
      ...
      for (j=0; j<nd->nucdata[i].nrates; j++) {
        ...
        for (k=0; k<rate->ninput; k++) {
          ...
          for (l=0; l<rate->ninput; l++) {
            if (l!=k) tempderiv *= y[rate->input[l]];
          }
...
#if defined(NETWORK_SCREENING) && !defined(NETWORK_SEP_YZ)
      {
        ...
        for (k=0; k<nd->nuc_count; k++) {
          ...
          row[k] += this_rhs.dYe * dyedyi
            ...
            + this_rhs.dYz * dyzdyi
#endif
            + this_rhs.dabar * dabardyi
            + this_rhs.dzbar * dzbardyi;
        }
      }
...
#if NETWORK_VARIABLE == NETWORK_VAR_RHO_TEMP
  {
    ...
    for (i=0; i<nd->nuc_count; i++) {
      ...
      row[i] += rhs_rho->dYe * dyedyi
        ...
        + rhs_rho->dYz * dyzdyi
#endif
        + rhs_rho->dabar * dabardyi
        + rhs_rho->dzbar * dzbardyi;
    }
...
#if DEBUG > 2
    myprintf("sparseness: %d / %zd (%g %%)\n", jacob->rowstart[nd->n_matrix], nd->n_matrix*nd->n_matrix, 100.0 * (double) jacob->rowstart[nd->n_matrix] / (double) (nd->n_matrix*nd->n_matrix));
#endif /* DEBUG */
  }
#endif /* NETWORK_SPARSE */
...
#error "unknown sparse matrix library"
  ...
  for (col_ind = offset, row_p = row;
       col_ind < n_row + offset;
       col_ind++, row_p++) {
...
static void compute_constant_screening_factors(struct network_rate *rate, const struct network_nucdata *nuc1, const struct network_nucdata *nuc2, const struct network_data *nd) {
  ...
  rate->tau_fac = pow( 27. / 2. * gsl_pow_2(M_PI) * (a1 * a2) / (a1 + a2) * gsl_pow_2(z1 * z2) * gsl_pow_4(ELECTRON_CHARGE_ESU) / (GSL_CONST_NUM_AVOGADRO * GSL_CONST_CGS_BOLTZMANN * gsl_pow_2(GSL_CONST_CGS_PLANCKS_CONSTANT_HBAR)), 1./3.);
...
void screening_apply(double rho, double temp, double ye, double yz, const struct network_data *nd, struct network_workspace *nw) {
  ...
  const network_var temp_var = {.v = temp, .dT = 1.0}, rho_var = {.v = rho, .drho = 1.0}, ye_var = {.v = ye, .dYe = 1.0}, yz_var = {.v = yz, .dYz = 1.0};
...
static network_var compute_screening( const struct network_rate *rate, network_var itemp13, network_var Gamma_prime, network_var lGamma_prime, network_var Gamma_prime14, network_var H_w_common) {
  ...
  if (Gamma.v >= 0.3) {
    ...
    C = n_ca(
             n_a(
                 n_a(
                     n_cm(Gamma_prime, 0.896434 * rate->z_tilde1),
  ...
  if (Gamma.v < 0.3) H = H_w;
  else if (Gamma.v > 0.8) H = H_s;
   ...
   */
  else H = n_a(n_m(H_w, n_cm(n_ca(Gamma, -0.8), -2.0)), n_m(H_s, n_cm(n_ca(Gamma, -0.3), 2.0)));
...
#if DEBUG > 3
    if (rate != &Be8_alpha_rate) myprintf("warning H is %g for Z_1 = %d, Z_2 = %d\n", H.v, nd->nucdata[rate->input[0]].nz, nd->nucdata[rate->input[1]].nz);
    else myprintf("warning H is %g for Be-8, He-4\n", H.v);
#endif /* DEBUG */
    H = zero_var;
  }
  assert(H.v >= 0.0);
