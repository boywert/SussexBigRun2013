...
static void network_solver_calc_dy( double temp, double rho, double dt, int steps, double *y, double *yn, const double *first_rhs, const jacob_t *old_jacob, const struct network_data *nd, struct network_workspace *nw, struct network_solver_trajectory *traj );
...
struct network_solver_data *network_solver_init( double tolerance, int matrixsize, int nelements, const struct network_nucdata *nucdata ) {
  ...
  for (i=0; i<nelements; i++) nsd->aion[i] = nucdata[i].na;
  ...
  for (i=0; i<nsd->maxstep; i++) nsd->a[i+1] = nsd->a[i] + nsd->steps[i+1];
  ...
  for (i=1; i<nsd->maxstep-1; i++) {
    if (nsd->a[i+1] > nsd->a[i]*nsd->alf[i*nsd->maxstep+i-1]) break;
  }
  nsd->maxiter = i+1;
...
void network_solver_integrate_traj( const struct network_data *nd, struct network_workspace *nw, struct network_solver_trajectory *traj ) {
	...
	for (i=0; i<nd->nuc_count; i++) traj->x[i] /= nd->nucdata[i].na;
...
static void network_solver_integrate_internal( double temp, double rho, double *y, double maxtime, const struct network_data *nd, struct network_workspace *nw, struct network_solver_trajectory *traj ) {
  ...
  for (i=0; i<nsd->nelements; i++) fwrite( &nsd->aion[i], sizeof(double), 1, fp );
#endif
  ...
  while (time < maxtime) {
...
#if defined (DEBUG) && defined (NETWORK_SEP_YZ)
    {
      ...
      for (i = 0; i < nd->nuc_count; i++) yz += y[i] * gsl_pow_2(nd->nucdata[i].nz);
...
#ifdef NETWORK_OUTPUT_TIMEEVOLUTION
    printf( "t(%03d): %11.5e, dt (%02d): %11.5e, x:", step, time, m, dttry ); for (i=0; i<nsd->nelements; i++) { printf( " %8.1e", y[i]*nsd->aion[i] ); } if (nsd->nelements<nsd->matrixsize) { printf( " %8.1e", y[nsd->nelements] ); } printf( "\n" );
	...
	if ( (traj->mintemp && temp < traj->mintemp) ||
	     (traj->maxtemp && temp > traj->maxtemp) ) {
		break;
	}
    }
...
#ifdef NETWORK_OUTPUT_TIMEEVOLUTION
  printf( "t(%03d): %11.5e, dt (%02d): %11.5e, x:", step, t, m, dttry ); for (i=0; i<nsd->nelements; i++) { printf( " %8.1e", y[i]*nsd->aion[i] ); } if (nsd->nelements<nsd->matrixsize) { printf( " %8.1e", y[nsd->nelements] ); } printf( "\n" );
...
static void network_solver_calc_dy( double temp, double rho, double dt, int steps, double *y, double *yn, const double *first_rhs, const jacob_t *old_jacob, const struct network_data *nd, struct network_workspace *nw, struct network_solver_trajectory *traj ) {
  ...
  for (j=1; j<steps; j++) {
    sanitize(yn, nd);
    if (traj) network_solver_interpolate_traj_rhotemp( traj, traj->time + j*h, yn, &rho, &temp );
  ...
  /* last step */
  sanitize(yn, nd);
  if (traj) network_solver_interpolate_traj_rhotemp( traj, traj->time + steps*h, yn, &rho, &temp );
  network_getrhs( rho, temp, yn, 0, nd, nw, rhs, deriv );
  for ( i=0; i<nMatrix; i++ ) { rhs[i] = h*rhs[i] - dy[i]; }
  dgstrs( TRANS, &L, &U, perm_c, perm_r, &B, &stat, &info );
  for ( i=0; i<nMatrix; i++ ) {
...
static void print_matrix(const int n, const double *matrix, const int *rowstart, const int *columns, const double *rhs, const int offset) {
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = 0; j < n; j++) {
      if (col == columns + rowstart[i+1] - offset) break;
      if (*col - offset == j) {
	fprintf(out, "%g ", *a);
	col++;
	a++;
      }
      else fprintf(out, "%g ", 0.);
    }
...
static void normalize(double *y, const struct network_solver_data *nsd) {
  ...
  sum = 0;
  for (i=0; i<nsd->nelements; i++) { sum += y[i]*nsd->aion[i]; }
  for (i=0; i<nsd->nelements; i++) y[i] /= sum;
}

static void sanitize(double *y, const struct network_data *nd) {
  double *yy, *yend = &y[nd->nuc_count];
  for (yy = y; yy != yend; yy++) *yy = fmin( 1.0, fmax( 1e-30, *yy ) );
...
#error "unknown sparse matrix library"
  ...
  for (i = 0; i < n_matrix; i++) {
    ...
    for (j = old_jacob->rowstart[i] - offset; j < old_jacob->rowstart[i+1] - offset; j++) {
      ...
      if (mod_jacob->columns[j] - offset == i) mod_jacob->values[j] += 1.0;
    }
  }
#else
  ...
  for (i = 0; i < n_matrix2; i++) {
    ...
    if (i % (n_matrix+1) == 0) mod_matrix[i] += 1.0;
  }
#endif /* NETWORK_SPARSE */
}
