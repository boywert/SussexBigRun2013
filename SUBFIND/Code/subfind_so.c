...
#endif
#ifdef SO_BAR_INFO
  double gas_mass, star_mass, temp, xlum; 
#endif
}
...
#ifdef SO_BAR_INFO
static double *gas_mass, *star_mass, *temp, *xlum; 
#endif
...
#endif
#ifdef SO_BAR_INFO
  gas_mass = mymalloc("GasMass", sizeof(double) * Ngroups);
  star_mass = mymalloc("GasMass", sizeof(double) * Ngroups);
  temp = mymalloc("Temperature", sizeof(double) * Ngroups);
  xlum = mymalloc("XrayLum", sizeof(double) * Ngroups); 
#endif
...
int subfind_overdensity_evaluate_barinfo(int target, int mode, int *nexport, int *nsend_local)
{
  ...
  n2ne = (HYDROGEN_MASSFRAC + 0.5 * (1 - HYDROGEN_MASSFRAC)) / (2 * HYDROGEN_MASSFRAC + 0.75 * (1 - HYDROGEN_MASSFRAC));
  gg = 1;
      ...
      while(startnode >= 0)
	{
	  ...
	  for(n = 0; n < ngb; n++)
	      ...
	      if(P[j].Type == 0)
		{
		  ...
		  particle_temperature = (MyFloat)Temperature * P[j].Mass;
#else
#ifdef COOLING
		  ne = SphP[j].Ne;
		  ...
		  particle_temperature = convert_u_to_temp(DMAX(All.MinEgySpec,
							   ...
							   SphP[j].d.Density * a3inv, &ne)* P[j].Mass;
#else
		  ...
		  this_temp += particle_temperature;
		  ...
		  */
		  ...
		  Lx = P[j].Mass * All.UnitMass_in_g / All.HubbleParam *
		       ...
		       pow(All.HubbleParam,2) * 
		       sqrt(particle_temperature) * cutoff;
