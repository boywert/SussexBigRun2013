...
#  pragma omp parallel for schedule (dynamic)	shared(halos, numHalos) private(i)
	...
	for (i = 0; i < numHalos; i++)
   {
     ...
     * c) have subhaloes 
     */
    if(halos[i].npart >= simu.AHF_MINPART && halos[i].stars_only.npart > 0 && halos[i].numSubStruct > 0)
     {
...
int spatialRef2halos(int num_refgrids, SPATIALREF **spatialRef)
{
  ...
  refgrid_start = 0;
     ...
     *------------------------------------------------*/
            ...
            if(halos[haloIndex].pos.x < ZERO && halos[haloIndex].pos.y < ZERO && halos[haloIndex].pos.z < ZERO)
             {
              halos[haloIndex].pos.x = spatialRef[i][j].centre.x;
              halos[haloIndex].pos.y = spatialRef[i][j].centre.y;
              halos[haloIndex].pos.z = spatialRef[i][j].centre.z;
...
#      pragma omp for schedule(dynamic)
 ...
 {
  FILE *fptmp;
  fptmp = fopen("PreliminaryHaloes.DAT","w");
#ifdef AHFpreliminaryhalosfile_WITHCOMMENTS
  ...
  for (ii = numHalos - 1; ii >= 0; ii--) {
    ...
    i = idx[ii];
		...
		fprintf(fptmp, "   #########################################################################\n");
#else
    fprintf(fptmp,"%d %d %16ld %16.8f %16.8f %16.8f %16.8f %16.8f %16d %16d %16d\n",
            ii,i,
            halos[i].npart,
...
rem_unbound(HALO *halo)
{
    ...
    I_now   = 0.0;
		...
		for (jpart = 0; jpart < halo->npart; jpart++) {
			...
			/* get particle velocity in halo rest frame velocity plus Hubble
			 *flow */
...
#ifdef SUSSEXBIGRUN
      cur_part->TotEnergy = 0.5*(Tpart*pow2(v_fac)+Upart*phi_fac)*m_fac;
#endif
...
#  endif
	...
	if (ndark > 0) {
		...
		/* get spin parameter of dark matter particles (all dark matter part's
		 *within sphere!) */
		absAngMom
		                       = sqrt(pow2(Lx_dark) + pow2(Ly_dark)
		           + pow2(Lz_dark));
		halo->DM_only.AngMom.x = Lx_dark / absAngMom;
		halo->DM_only.AngMom.y = Ly_dark / absAngMom;
		halo->DM_only.AngMom.z = Lz_dark / absAngMom;
		halo->DM_only.lambda   = absAngMom / M_dark / sqrt(
		    2. * halo->M_vir * halo->R_vir);
...
void exciseSubhaloStars(HALO *halos, long ihost)
{
  ...
  if(khoststar != nhoststar) {
    fprintf(stderr,"exciseSubhaloStars: number of stars in host does not add up: khoststar=%ld nhoststar=%ld\n",khoststar,nhoststar);
    common_terminate(EXIT_FAILURE);
  }
  ...
  if(nsubstar > 0)
   {
    ...
    if(idsubstar == NULL || idhoststar == NULL) {
      fprintf(stderr, "Not enough memory to re-allocate arrays in %s (nhoststar=%ld, nsubstar=%;d).  Aborting.\n\n", __func__,nhoststar,nsubstar);
      common_terminate(EXIT_FAILURE);
    }
