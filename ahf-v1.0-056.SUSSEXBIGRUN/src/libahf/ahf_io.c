...
#ifdef SUSSING2013
          fprintf(fout, "%" PRIpartid " %g %g %g %g %g %g %g\n",
                  current->id,
                  current->E,
  ...
  double        Tnow, Tformation;
    ...
    if(halos[i].ignoreme == FALSE) {
      ...
      if ((halos[i].npart >= simu.AHF_MINPART)) {
        ...
        /* start with dumping the actual number of particle IDs to follow... */
        fprintf(fout, "%lu\n", (unsigned long)(halos[i].stars_only.npart));
  ...
  double        Tnow, Tformation;
    ...
    if(halos[i].ignoreme == FALSE) {
      ...
      if ((halos[i].npart >= simu.AHF_MINPART)) {
        ...
        /* start with dumping the actual number of particle IDs to follow... */
        fprintf(fout, "%lu\n", (unsigned long)(halos[i].npart_uniquestars));
...
#ifdef SUSSEXBIGRUN
	  particle_energy = (float) cur_part->TotEnergy;
	  fwrite(&particle_energy, sizeof(float), 1, f);
#else
